...
echo "$($PSQL "SELECT name FROM teams INNER JOIN games ON team_id=winner_id OR team_id=opponent_id WHERE round='Eighth-Final' AND year=2014 ORDER BY name ASC")"

echo -e "\nList of unique winning team names in the whole data set:"
echo "$($PSQL "SELECT name FROM teams INNER JOIN games ON team_id=winner_id GROUP BY name ORDER BY name asc")"
